...
// Je nach Status des Segments, unterschiedliche Behandlung bei nicht Vorhandensein
  ...
  nSearchEnd   := IIF(lTheRFFs, 2, nUNT)
  ...
  nSearchEnd   := IIF(nSearchEnd < 0, 0, nSearchEnd)
   ...
   IF nFound = 0
      ...
      Else
         ...
         IF !lOk
            ...
            ELSE
               AADD(aErrorList, "Segment: " + cDummy + " fehlt.")
            ENDIF
            lOk := EXTRAINFO(-1)
         ENDIF
      ENDIF
...
//  FWRITE(nOut, "Die Rechnung weist keine rechnerischen Fehler auf." + xCrLf + xCrLf)
 ELSE
